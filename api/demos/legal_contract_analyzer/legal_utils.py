...
class LegalRAGPipeline:
    ...
    async def generate_answer_stream(self, question: str, relevant_chunks: List[DocumentChunk]) -> AsyncGenerator[str, None]:
        """
        Generate a streaming legal analysis using the LLM and relevant chunks
        ...
        Yields:
            Chunks of the generated legal analysis
        """
        try:
            if not relevant_chunks:
                yield "I don't have enough information to answer your legal question. Please make sure the document has been processed and contains relevant content."
                return
            ...
            # Create legal-specific RAG prompt
            prompt = f"""You are a legal AI assistant analyzing a legal document. Provide accurate, helpful legal analysis based on the document content.
...
Instructions:
1. Answer as a legal AI assistant - be professional, accurate, and helpful
2. ONLY use information from the provided document context above
3. If the context doesn't contain relevant information, say "I don't have specific information about that in the document. Please consult with a legal professional for detailed advice."
4. Focus on legal implications, risks, and key terms
5. Be concise but comprehensive in your analysis
6. Do not provide legal advice - only analysis of the document content
7. If discussing risks, mention specific clauses or terms from the document

Legal Analysis:"""
            
            # Generate streaming answer
            async for chunk in self.llm_provider.generate_stream(prompt, temperature=0.1, max_tokens=500):
                yield chunk
                
        except Exception as e:
            logger.error(f"Error generating streaming legal analysis: {e}")
            yield f"Sorry, I encountered an error while analyzing your legal question: {str(e)}"
